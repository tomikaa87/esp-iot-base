...
{
    std::array<uint16_t, 256> Crc16Ccitt{
...
void Hash::Crc16::update(const uint8_t* data, size_t size)
{
    for (; size > 0; --size) {
        const uint8_t pos = static_cast<uint8_t>((_crc >> 8) ^ *data++);
        _crc = static_cast<uint16_t>((_crc << 8) ^ LookupTable::Crc16Ccitt[pos]);
    }
}

void Hash::Crc16::update(const uint8_t data)
{
    const uint8_t pos = static_cast<uint8_t>((_crc >> 8) ^ data);
    _crc = static_cast<uint16_t>((_crc << 8) ^ LookupTable::Crc16Ccitt[pos]);
}

uint16_t Hash::Crc16::value() const
{
    return _crc;
}
